...
# -------------------------------------------------
scenario <- 4                         # 1,2,3,4 
rho_w <- 0.3                          # Within group corr : 0.8, 0.5, 0.3
rho_b <- 0.01                         # between group corr : 0.2, 0.1, 0.01
n <- c(500,500,300,200)[scenario]     # sample size
n_iter <- 60000                       # total mcmc per chain
burnin <- 30000                       # burnin samples
n_reps <- 5                           # replications

# Tuned proposal sigmas for GIGG (replace with your actual tuned values)
sigma_prop_a <- 5
sigma_prop_b <- 1.5

# output file (different from BGHS/GRASP)
results_file <- "GIGG_results.csv"
...
library(BayesLogit)
library(MCMCpack)
library(GIGrvg)   # required for GIGG

# Helper: create initial vector for GIGG (zero start)
...
# --------------------------------------------------
cat("\n============================================================\n")
cat(sprintf("GIGG: Scenario %d, rho_w=%.1f, rho_b=%.2f, n=%d\n", scenario, rho_w, rho_b, n))
...
for(rep in 1:n_reps){
  ...
  init <- create_gigg_init(n_obs, p, G, group_sizes, type = "zero")
  
  # 3. Run GIGG Gibbs sampler
  posterior <- gigg_gibbs(data = dat$data,
                          nsamples = n_iter,
                          burnin = burnin,
                          init = init,
